...
int main()
{
    ...
    {
        //Create window
        window = SDL_CreateWindow( "SDL Tutorial", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 608, 540, SDL_WINDOW_SHOWN );
        if( window == NULL )
        {
            ...
            //surface = SDL_SetVideoMode(W, H, 32, 0);// NO ES ACTUAL!!!

            //SDL_EnableKeyRepeat(150, 30);  // NO ES ACTUAL!!!
                ...
                if (moving) {
                    ...
                    for (unsigned s = 0; s < sect->npoints; ++s)
                        if (IntersectBox(px, py, px + dx, py + dy, vert[s + 0].x, vert[s + 0].y, vert[s + 1].x,
                            ...
                            float hole_low =
                                    sect->neighbors[s] < 0 ? 9e9 : max(sect->floor, sectors[sect->neighbors[s]].floor);
                            float hole_high =
                                    sect->neighbors[s] < 0 ? -9e9 : min(sect->ceil, sectors[sect->neighbors[s]].ceil);
